...
const DB_SOURCE_EXT =
    process.env.DB_SOURCE_EXT || (NODE_ENV === 'develop' ? 'ts' : 'js');
const sourceRootKey =
    DB_SOURCE_EXT === 'ts' || NODE_ENV === 'develop'
        ? 'sourceRoot'
        : 'outputPath';
...
const projects = Object.keys(nestCliConfig.projects).map(
    key => nestCliConfig.projects[key],
);
const libs = Object.keys(nestCliConfig.projects)
    .filter(key => nestCliConfig.projects[key].projectType === 'library')
    .map(key => nestCliConfig.projects[key]);
const apps = Object.keys(nestCliConfig.projects)
    .filter(key => nestCliConfig.projects[key].projectType === 'application')
    .map(key => nestCliConfig.projects[key]);
...
if (connectionString.protocol === 'sqlite') {
    ...
    module.exports = {
        type: 'sqlite',
        database: dbFile,
        entities: [...libs, ...apps].map(
            lib =>
                `${
                    lib[sourceRootKey]
                }/**/entities/**/*.entity.${DB_SOURCE_EXT}`,
        ),
        migrations: [...libs, ...apps].map(
            lib => `${lib[sourceRootKey]}/**/migrations/**/*.${DB_SOURCE_EXT}`,
        ),
        subscribers: [...libs, ...apps].map(
            lib => `${lib[sourceRootKey]}/**/subscribers/**/*.${DB_SOURCE_EXT}`,
        ),
        logging: 'all',
        synchronize: false,
        cli: {
